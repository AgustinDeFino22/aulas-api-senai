...
class ProdutosExFix {
    async adicionar(req, resp) {
        try{
            ...
            if (!novoProduto.nome || !novoProduto.descricao || !novoProduto.preco || !novoProduto.estoque) {
                resp
                    .status(400)
                    .send("Os campos nome, preço e estoque são obrigatórios.");
                return;
            }
    ...
    async atualizar(req, resp) {
        try {
            ...
            if (!produtoEditar.id || !produtoEditar.nome || !produtoEditar.descricao || !produtoEditar.preco || !produtoEditar.estoque) {
                resp.status(400).send("Os campos id, nome, descrição, preço e estoque são obrigatórios.");
                return;
            }
